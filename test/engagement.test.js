...
test('저장 값은 정규화하고 예전 저장분도 그대로 열린다', () => {
  ...
  assert.match(app, /'draftReview', 'projectNarrative', 'engagement', 'proposalTables', 'preciseReview', 'submissionIncluded'\]/);
...
test('의뢰 건 화면은 기존 화면을 대체하지 않고 덧붙는다', () => {
  assert.match(app, /tools = \{ home: homeView, coaching: coachingView, applicants: applicantsToolView, sample: sampleView, engagement: engagementView \}/);
