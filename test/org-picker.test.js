...
test('펼침 기억은 이번 화면에서만 산다', () => {
  ...
  assert.match(save, /openAddForms: \[\], openFitGroups: \[\], openSections: \{\}, openOrgGroups: undefined, openOrgYears: \[\], quickFilledFrom: \{\}/);
  const load = app.slice(app.indexOf('const restored = {'), app.indexOf('const restored = {') + 2000);
  assert.match(load, /openAddForms: \[\], openFitGroups: \[\], openSections: \{\}, openOrgGroups: undefined, openOrgYears: \[\], quickFilledFrom: \{\}/);
});
