...
import { shouldRunAgain } from '../src/ai-rerun.js';

const read = name => fs.readFileSync(new URL(name, import.meta.url), 'utf8').split('\r\n').join('\n');
const app = read('../src/app.js');

test('만들어 둔 것이 없으면 만들고, 있으면 만들지 않는다', () => {
  assert.equal(shouldRunAgain({ hasResult: false }), true);
  assert.equal(shouldRunAgain({ hasResult: true }), false);
  // 아무것도 넘기지 않아도 「없다」로 본다 — 첫 호출을 막지 않는다.
  assert.equal(shouldRunAgain(), true);
  assert.equal(shouldRunAgain({}), true);
});

test('다시 만들라는 뜻인 자리는 언제나 만든다', () => {
  for (const hasResult of [false, true]) {
    for (const inputChanged of [false, true]) assert.equal(shouldRunAgain({ redo: true, hasResult, inputChanged }), true);
  }
});

test('공고가 바뀌었으면 앞 공고의 설계를 쓰지 않는다', () => {
  assert.equal(shouldRunAgain({ hasResult: true, inputChanged: true }), true);
  assert.equal(shouldRunAgain({ hasResult: true, inputChanged: false }), false);
});
...
test('★ 결과가 있는 상태에서 두 번 눌러도 호출은 한 번이다', () => {
  ...
  const press = ({ redo = false } = {}) => {
    if (!shouldRunAgain({ redo, hasResult: Boolean(state.master), inputChanged: Boolean(state.analysedTitle) && state.analysedTitle !== state.noticeTitle })) return;
    calls += 1;
...
test('판정이 돈 드는 걸음보다 먼저 온다', () => {
  ...
  const guard = region.indexOf('shouldRunAgain(');
