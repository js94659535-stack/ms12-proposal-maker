...
**대시보드와 `wrangler.toml`에 같은 이름을 두면 배포가 실패할 수 있다. `87b1042`가 그렇게 실패했다.**
`wrangler.toml`의 `[vars]`와 같은 이름이 대시보드에 **암호(Secret)**로도 있으면 빌드가 죽는다.
중복된 암호 하나를 지우자 같은 `wrangler.toml`로 `4130a0b`가 성공했다.
같은 이름을 일반 변수(plain text)로 두는 것은 빌드를 죽이지는 않지만, **주인이 둘이 된다.**
주인은 `wrangler.toml` 하나로 둔다 — git 이력에 남고, 시험이 그 파일을 직접 읽어 확인한다
(`test/ai-usage.test.js`의 「배포 설정의 단가를 priceOf가 실제로 읽는다」).
