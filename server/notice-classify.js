...
const RULES = [
  ...
  { fitness: FITNESS.proposal, pattern: /공모|지원\s*사업|배분\s*사업|사업\s*신청|신청\s*접수|공모\s*사업|지원\s*신청|사업\s*공고|프로그램\s*공모|기금\s*지원|참여\s*기관\s*모집|수행\s*기관\s*모집|협력\s*기관\s*모집|배분\s*신청|위원\s*모집|위원\s*공모|위원\s*위촉/ }
];
