...
async function signup(db, request, body) {
  ...
  const now = new Date();
  const clientHash = await sha256Hex(`signup:${request.headers.get('cf-connecting-ip') || 'local'}`);
  const emailHash = await sha256Hex(`signup-email:${checked.value.email}`);
